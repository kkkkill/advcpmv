...
/* BEGIN progress mod */
static void file_progress_bar ( char * _cDest, int _iBarLength, double _iProgress, double _iTotal )
{
  // write number to progress bar
  double fPercent = ( double ) _iProgress / _iTotal * 100.f;
  ...
  {
    if ( fPercent > ( double ) ( i - 1 ) / ( _iBarLength - 10 ) * 100.f )
      _cDest[i] = '|';
    else
      _cDest[i] = '-';
  }
...
double file_size_format ( char * _cDst, double _iSize, long _iCounter )
{
  long iCounter = _iCounter;
  double dSize = _iSize;
  while ( dSize >= 1000. )
  {
    dSize /= 1024.;
            ...
            )
{
  ...
  /* filesize */
  double file_size = max_n_read;
  struct stat file_stat;
  if (fstat(src_fd, & file_stat) == 0) file_size = file_stat.st_size;
  ...
  double last_size = g_iTotalWritten;
  /* END progress mod */
  ...
  while (max_n_read)
    ...
    if (progress) {
      ...
      if ( iCountDown == 0 )
      {
        ...
        double cur_size = g_iTotalWritten + *total_n_read / 1024;
        ...
        double copy_speed = ( ( double ) ( cur_size - last_size ) / sec_elapsed );
        if (copy_speed < 0) copy_speed = 0;
        ...
        int sec_remaining = ( int ) ( ( double ) isec_elapsed / cur_size * g_iTotalSize ) - isec_elapsed;
